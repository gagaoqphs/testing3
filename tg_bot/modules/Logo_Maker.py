...
async def lego(event):
 ...
 try:
    ...
    img = Image.open('./Ayaka/resources/Blankmeisnub.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "gold"
    shadowcolor = "blue"
    font = ImageFont.truetype("./Ayaka/resources/Chopsic.otf", 160)
    ...
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By Ayaka")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Error Report @ToxicBotChat, {e}')
